...
class UnboundBuffer {
 public:
  ...
  void* const ptr;//不能修改 ptr 所指向的地址值，只能修改其所指向内存的内容。
  const size_t size;
  ...
  // Aborts a pending waitRecv call. yed 立即终止之前已经启动但未完成的 waitRecv 和 waitSend 操作
  ...
  // the buffer w.r.t. the offset.该变量用于表示将要发送或接收的字节数，如果未指定具体字节数，则默认为缓冲区剩余的字节数量

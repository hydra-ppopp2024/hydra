...
void allreduce(const detail::AllreduceOptionsImpl& opts) {
  ...
  // Sanity checks
  GLOO_ENFORCE_GT(out.size(), 0);//检查是否大于某个值
  ...
  for (size_t i = 0; i < out.size(); i++) {//容器内元素个数，这里是指针指向的地址区域，它的大小应该等于~
    GLOO_ENFORCE_EQ(out[i]->size, totalBytes);
  }
  for (size_t i = 0; i < in.size(); i++) {
    GLOO_ENFORCE_EQ(in[i]->size, totalBytes);
  }
          // std::cout << "yedallre"<< std::endl;
  // std::cout << "athoter" << opts.algorithm << std::endl;
  ...
  if (context->size == 1) {
    reduceInputs(0, totalBytes);//对应偏移量和length
...
//
  ...
  GLOO_ENFORCE(
      ...
      "missing connection between rank " + std::to_string(context->rank) +
          " (this process) and rank " + std::to_string(sendRank));//这里没看到对getpair的具体定义
  ...
  const size_t numSegments = roundUp(
      std::max(
          (totalBytes + (maxSegmentBytes - 1)) / maxSegmentBytes,//总字节数除以最大块子节数，得到块数，注意这里要求必须是节点数的整数倍
          (size_t)context->size * 2),
      (size_t)context->size);
  GLOO_ENFORCE_EQ(numSegments % context->size, 0);
  GLOO_ENFORCE_GE(numSegments, context->size * 2);//分块，大于两块，很合理
  const size_t numSegmentsPerRank = numSegments / context->size;
  const size_t segmentBytes =
      roundUp((totalBytes + numSegments - 1) / numSegments, opts.elementSize);//roundup(n,m)  return ((n + m - 1) / m) * m; roundUp(56, 8) 会返回结果 64 确保任务均匀？

  // Allocate scratch space to hold two chunks
  //key code yed
  std::unique_ptr<uint8_t[]> tmpAllocation(new uint8_t[segmentBytes * 2]);//new uint8_t[segmentBytes * 2] 是一个动态内存分配操作,tmpAllocation 是对 std::unique_ptr<uint8_t[]> 类型对象的命名
  std::unique_ptr<transport::UnboundBuffer> tmpBuffer =
      context->createUnboundBuffer(tmpAllocation.get(), segmentBytes * 2);//get() 用于将智能指针所管理的内存资源传递给 context->createUnboundBuffer() 方法
  ...
  std::array<size_t, 2> segmentOffset;//两个索引
  segmentOffset[0] = 0;
  ...
  for (auto i = 0; i < (numSegments - numSegmentsPerRank + 2); i++) {
    if (i >= 2) {
      ...
      if (prev.recvLength > 0) {
        ...
        // std::cout << "yedwaitrecvbegin "<< std::endl;
        // Wait for segment from neighbor.
        tmp->waitRecv(opts.timeout);
        // std::cout << "yedwaitrecvend "<< std::endl;        
        ...
        opts.reduce(
            ...
            static_cast<const uint8_t*>(tmp->ptr) + segmentOffset[i & 0x1],//i 为偶数时，i & 0x1 的结果为 0，否则为1，tmp收了两块
            prev.recvLength / opts.elementSize);
      }
          //  std::cout << "n=== "<<prev.recvLength / opts.elementSize<< std::endl;     
      //yedcode 
...
// //
    ...
    if (i < (numSegments - numSegmentsPerRank)) {
      ...
      //   {
      //     // std::cout << "reduce out send begin 1++++++ " << std::endl;
      //     int* outtemp = (int*)(out[0]->ptr + cur.sendOffset);
      //     for(int i = 0; i < cur.sendLength/sizeof(int); i++){
      //       std::cout << outtemp[i] << "  ";
      //     }
      //     std::cout << std::endl;
      //     // std::cout << "reduce out send begin 2++++++ " << std::endl;
      // }
      }
    }
  }

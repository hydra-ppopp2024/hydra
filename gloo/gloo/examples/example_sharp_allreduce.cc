...
int main(void) {
  ...
  auto fileStore = gloo::rendezvous::FileStore("/home/mpi_share/env/yed_gloo/tmp");
  ...
  float *inputPointers = reinterpret_cast<float*>(malloc(sizeof(float) * 100));
  float *outputPointers = reinterpret_cast<float*>(malloc(sizeof(float) * 100));

  

  // Configure AllreduceOptions struct
  gloo::SharpAllreduceOptions opts_(context,ibname.c_str());
  // opts_.setIbname();
  opts_.setInput(inputPointers, 100);
  opts_.setOutput(outputPointers, 100);
  for (int i = 0; i < 100; i++) {//压入数据
    inputPointers[i] = i * (rank + 1.0);
    outputPointers[i] = 0;
  }
  // gloo::sharp_allreduce_init(opts_);
  gloo::sharp_allreduce(opts_);
  // gloo::sharp_allreduce_destroy();

...
// //
  ...
  std::string prefix = getenv("PREFIX");
    std::string prefix2 = getenv("PREFIX2");
  auto prefixStore = gloo::rendezvous::PrefixStore(prefix, fileStore);
  auto prefixStore2 = gloo::rendezvous::PrefixStore(prefix2, fileStore);
  ...
  fprintf(stdout, "connectFullMesh START.\n");
  context->connectFullMesh(prefixStore, dev);
    gloo::transport::tcp::attr attr2;
  attr2.iface = "eth0";
  attr2.ai_family = AF_UNSPEC;  // Use either (default)
    auto dev2 = gloo::transport::tcp::CreateDevice(attr2);
  auto context2 = std::make_shared<gloo::rendezvous::Context>(rank, size);
  context2->connectFullMesh(prefixStore2, dev2);
...
// /*
  ...
  float *outputPointers = reinterpret_cast<float*>(malloc(sizeof(float) * 20));
  float *inputPointers2 = reinterpret_cast<float*>(malloc(sizeof(float) * 10));
  float *outputPointers2 = outputPointers+10;
  // std::array<float, 100> inputPointers;
  // std::array<float, 100> outputPointers;
  printf("rank:%d size:%d connected full mesh3\n", rank, size);    
    gloo::SharpAllreduceOptions opts2_(context2,ibname.c_str());
  printf("rank:%d size:%d connected full mesh2\n", rank, size);
  ...
  // float *inputPointers2 = inputPointers+5;
  // float *outputPointers2 = outputPointers+5;
    opts2_.setInput(inputPointers2, 10);
    opts2_.setOutput(outputPointers2, 10);
  ...
  opts_.setReduceFunction(fn);
    printf("rank:%d size:%d connected full mesh4\n", rank, size);
  for (int i = 0; i < 10; i++) {//压入数据
    inputPointers[i] = i * (rank + 1.0);
  }
  for (int i = 0; i < 10; i++) {//压入数据
    inputPointers2[i] = i * (rank + 1.0);
  }
  for (int i = 0; i < 20; i++) {//压入数据
    outputPointers[i] = 0;
  }
  ...
  for (int i = 0; i < 10; i++) {
    std::cout << "data[" << i+10 << "] = " << inputPointers2[i] << std::endl;   
  }

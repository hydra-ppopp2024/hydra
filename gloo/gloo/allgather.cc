...
void allgather(AllgatherOptions& opts) {
  ...
  // The chunk size may not be divisible by 2; use dynamic lookup.可能不被2整除
  std::array<size_t, 2> chunkSize;
  chunkSize[0] = inBytes / 2;
  chunkSize[1] = inBytes - chunkSize[0];
  std::array<size_t, 2> chunkOffset;
  chunkOffset[0] = 0;
  chunkOffset[1] = chunkSize[0];//为什么要除以2

  for (auto i = 0; i < (context->size - 1) * 2; i++) {
    const size_t sendSegment = context->size + context->rank - (i / 2);//为什么有0.5这种东西？
    const size_t recvSegment = sendSegment - 1;
    size_t sendOffset =
        ((sendSegment * inBytes) + chunkOffset[i & 0x1]) % outBytes;//i奇ib奇，chunk0.5，send0.5，奇ib偶，全偶
    size_t recvOffset =

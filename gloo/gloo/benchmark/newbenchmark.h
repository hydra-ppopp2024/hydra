...
class newBenchmark {
 public:
  newBenchmark(
    ...
    struct options& options)
      : context_(context),context2_(context2),
        options_(options) {}

  virtual ~newBenchmark() {}
 ...
 protected:
  virtual std::vector<T*> allocate(int inputs, size_t elements) {
    ...
    for (int i = 0; i < inputs; i++) {
      ...
      for (size_t j = 0; j < elements; j++) {
        memory[j] = (j * stride) + value;//这里的是数据源吗？
      }
  ...
  std::shared_ptr<::gloo::Context> context2_;  
  struct options options_;
  std::unique_ptr<::gloo::Algorithm> algorithm_;
  std::vector<std::vector<T, aligned_allocator<T, kBufferAlignment>>>//aligned_allocator 是一个自定义的分配器类，用于在内存分配时强制要求对齐。
      inputs_;
};

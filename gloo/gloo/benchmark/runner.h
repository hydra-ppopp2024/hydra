...
class Runner {
 public:
  ...
  template <typename T>      
  using BenchmarkFn2 = std::function<std::unique_ptr<newBenchmark<T>>(
      std::shared_ptr<::gloo::Context>&,std::shared_ptr<::gloo::Context>&)>;      
#endif
...
#if GLOO_HAVE_TRANSPORT_GLEX_RDMA_S
  template <typename T>
  void run2(BenchmarkFn2<T>& fn);

  template <typename T>
  void run2(BenchmarkFn2<T>& fn, size_t n);
#endif
 protected:
#if GLOO_USE_REDIS
  ...
  #if GLOO_HAVE_TRANSPORT_GLEX_RDMA_S
  std::vector<std::shared_ptr<transport::Device>> transportDevices2_;  
   std::shared_ptr<rendezvous::ContextFactory> contextFactory2_;  
  #endif 

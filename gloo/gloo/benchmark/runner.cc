...
#if GLOO_HAVE_TRANSPORT_TCP && !GLOO_HAVE_TRANSPORT_GLEX_RDMA_S
...
#if GLOO_HAVE_TRANSPORT_GLEX
  if (options_.transport == "glex")  {
    //todo 完成GLEX的benchmark初始化部分
    transportDevices_.push_back(transport::glex::CreateDevice());
  }
#endif

#if GLOO_HAVE_TRANSPORT_GLEX_RDMA_T && !GLOO_HAVE_TRANSPORT_GLEX_RDMA_S
  if (options_.transport == "glex_rdma_t")  {
    //todo 完成GLEX_RDMA_T的benchmark初始化部分
    transportDevices_.push_back(transport::glex_rdma_t::CreateDevice());
  }
#endif
...
//       if (options_.tcpDevice.empty()) {
//       transport::tcp::attr attr;
//       transportDevices2_.push_back(transport::tcp::CreateDevice(attr));
//     } else {
//       for (const auto& name : options_.tcpDevice) {
//         transport::tcp::attr attr;
//         attr.iface = name;
...
#if GLOO_HAVE_TRANSPORT_GLEX_RDMA_S
  if (options_.transport2 == "glex_rdma_t")  {
    //todo 完成GLEX_RDMA_T的benchmark初始化部分
    transportDevices2_.push_back(transport::glex_rdma_t::CreateDevice());
      if (options_.tcpDevice.empty()) {
      transport::tcp::attr attr;
      transportDevices_.push_back(transport::tcp::CreateDevice(attr));
    } else {
      ...
      if (options_.tcpDevice2.empty()) {
      transport::tcp::attr attr2;
      transportDevices2_.push_back(transport::tcp::CreateDevice(attr2));
    } else {
      for (const auto& name : options_.tcpDevice2) {
        transport::tcp::attr attr2;
        attr2.iface = name;
        transportDevices2_.push_back(transport::tcp::CreateDevice(attr2));
      }
...
void Runner::rendezvousRedis() {
  ...
  contextFactory_ = std::make_shared<rendezvous::ContextFactory>(
      backingContext);  
#if GLOO_HAVE_TRANSPORT_GLEX_RDMA_S
    rendezvous::PrefixStore prefixStore2(options_.prefix+"1", redisStore);  
  auto backingContext2 = std::make_shared<rendezvous::Context>(
      options_.contextRank, options_.contextSize);    
  backingContext2->connectFullMesh(prefixStore2, transportDevices2_.front());       
  contextFactory2_ = std::make_shared<rendezvous::ContextFactory>(
      backingContext2);    
#endif  
}
#endif
...
#if GLOO_HAVE_TRANSPORT_GLEX_RDMA_S
  rendezvous::PrefixStore prefixStore2(options_.prefix+"1", fileStore);
  auto backingContext2 = std::make_shared<rendezvous::Context>(
    options_.contextRank, options_.contextSize);  
  backingContext2->connectFullMesh(prefixStore2, transportDevices2_.front());  
  contextFactory2_ = std::make_shared<rendezvous::ContextFactory>(
...
#if GLOO_HAVE_TRANSPORT_GLEX_RDMA_S
template <typename T>
void Runner::run2(BenchmarkFn2<T>& fn) {
  printHeader();

  if (options_.elements > 1) {
    run2(fn, options_.elements);
    return;
  }
...
#if GLOO_HAVE_TRANSPORT_GLEX_RDMA_S
template <typename T>
void Runner::run2(BenchmarkFn2<T>& fn, size_t n) {
  std::vector<std::unique_ptr<newBenchmark<T>>> benchmarks;
  ...
  for (auto i = 0; i < options_.threads; i++) {
    ...
    auto context2 = contextFactory2_->makeContext(
        transportDevices2_[i % transportDevices2_.size()]);
    context2->base = options_.base;    
    auto benchmark = fn(context,context2);
...
void Runner::printHeader() {
  ...
  std::cout << std::setw(11) << ("p995 " + suffix);
...
void Runner::printDistribution(
  ...
  auto avgtime = latency.sum() / latency.size();
  ...
  std::cout << std::setw(11) << (latency.percentile(0.995) / div);
  std::cout << std::setw(11) << (latency.max() / div);
  std::cout << std::setw(11) << avgtime / div;
...
#if GLOO_HAVE_TRANSPORT_GLEX_RDMA_S
template void Runner::run2(BenchmarkFn2<char>& fn);
template void Runner::run2(BenchmarkFn2<char>& fn, size_t n);
template void Runner::run2(BenchmarkFn2<float>& fn);
template void Runner::run2(BenchmarkFn2<float>& fn, size_t n);
#endif

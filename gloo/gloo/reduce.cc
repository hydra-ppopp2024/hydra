...
void reduce(ReduceOptions& opts) {
  ...
  transport::UnboundBuffer* in = opts.in.get();//这里get到的是指针类型
...
const size_t segmentBytes = roundUp(
      std::min(
          ...
          maxSegmentSize),
      opts.elementSize);
      // std::cout<< (totalBytes + (context->size * 2 - 1)) / (context->size * 2)<<std::endl;
      // std::cout<<maxSegmentSize<<std::endl;
      // std::cout<<opts.elementSize<<std::endl;
      // std::cout<<"init fin"<<segmentBytes<<std::endl;
  ...
  //         (totalBytes + (context->size * 2 - 1)) / (context->size * 2),
  //     opts.elementSize);
      // std::cout<< (totalBytes + (context->size * 2 - 1)) / (context->size * 2)<<std::endl<<maxSegmentSize<<std::endl<<opts.elementSize<<std::endl;
      // std::cout<<"init fin"<<segmentBytes<<std::endl;
